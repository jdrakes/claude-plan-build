...
The request is a one-line bug report about column order. In the scaffolded
fixture the column order comes from src/cli.py, which falls back to the keys
of the first row when no --columns flag is given, so this is one edit or a
one-line answer about the existing flag.
